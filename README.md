...
进入对话页，点击「对话 / 轨迹」标签旁的「插件中心」：

- 列表实时读取 GitHub 上声明了 `dsh` 清单的仓库，按星标数降序排列。
- 卡片显示图标、名称、版本、简介、状态（未安装 / 已启用 / 已停用 / 可更新），并提供安装 / 启用 / 停用 / 更新 / 卸载按钮；点击卡片展开详情。
- 进入 **设置 → 插件 → 插件配置 → 插件中心**，用开关启用/停用该功能。
