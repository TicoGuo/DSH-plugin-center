...
export const DEFAULT_GITHUB_QUERY = 'topic:dsh-plugin OR dsh-plugin in:name,description OR deepseek-harness in:name,description,readme';
 ...
 */
export function parseGithubSearchResponse(json) {
    ...
    for (const item of items) {
        ...
        if (typeof fullName !== 'string' || fullName === '')
            continue;
        if (typeof stars !== 'number' || typeof htmlUrl !== 'string')
            continue;
        summaries.push({
            fullName,
 ...
 */
export function parseGithubPluginManifest(owner, repo, stars, json) {
    if (json === null || typeof json !== 'object' || Array.isArray(json))
        return null;
    const record = json;
    ...
    const requirements = peerDependencies !== null
 ...
 */
export async function githubCatalog(query, fetchFn) {
