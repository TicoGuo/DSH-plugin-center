...
function isTrustedOrigin(value, host) {
    try {
        ...
        if (TRUSTED_ORIGIN_HOSTS.has(hostname))
            return true;
...
function crossOriginReason(req) {
    ...
    if (typeof referer === 'string' && referer !== '') {
        if (isTrustedOrigin(referer, host))
            return null;
...
export function apply(ctx, config = {}) {
    ...
    const ensureCatalog = (catalogUrl) => {
        if (registryLoading)
            return registryLoad ?? Promise.resolve();
    ...
    const resolveLatestVersion = async (packageName) => {
        ...
        if (cached !== undefined && Date.now() - cached.at < VERSION_TTL_MS)
        ...
        if (version !== null)
            versionCache.set(packageName, { version, at: Date.now() });
        return version;
    };
    ...
    async function dispatch(req, res) {
        ...
        try {
            ...
            if (sub === '/install' || sub === '/update' || sub === '/uninstall' || sub === '/enable' || sub === '/disable') {
                ...
                if (sub === '/install')
                    sendJson(res, 200, await exclusive(() => installOne(id)));
                else if (sub === '/update')
                    sendJson(res, 200, await exclusive(() => updateOne(id)));
                else if (sub === '/uninstall')
                    sendJson(res, 200, await exclusive(() => uninstallOne(id)));
                else if (sub === '/enable')
                    sendJson(res, 200, await exclusive(() => setEnabled(id, true)));
                else
                    sendJson(res, 200, await exclusive(() => setEnabled(id, false)));
                return;
            }
