...
const AWESOME_CATALOG_URL = "https://raw.githubusercontent.com/awesome-dsh-plugin/awesome-dsh-plugin/main/README.zh.md";
...
function parseAwesomeReadme(markdown) {
	...
	for (const line of markdown.split("\n")) {
		...
		if (owner === void 0 || owner === "" || repo === void 0 || repo === "") continue;
...
*/
function writeProfileState(profileDir, manifest, disabledNames, packages) {
...
function apply(ctx, config = {}) {
	...
	const installOrUpdate = async (action, id) => {
		...
		try {
			...
			const packageName = [...after.plugins.installedNames].filter((name) => !before.plugins.installedNames.has(name))[0] ?? resolvedName(after, entry);
	...
	const setEnabled = async (id, enabled) => {
		...
		try {
			const loaded = readProfileState(profileDir);
			const packageName = resolvedName(loaded, entry);
			writeProfileState(profileDir, enabled ? withBundleEnabled(loaded.manifest, packageName) : withBundleDisabled(loaded.manifest, packageName), enabled ? withoutDisabled(loaded.plugins.disabledNames, packageName) : withDisabled(loaded.plugins.disabledNames, packageName), loaded.plugins.packages);
