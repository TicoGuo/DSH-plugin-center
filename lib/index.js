...
function apply(ctx, config = {}) {
	...
	const uninstallOne = async (id) => {
		...
		try {
			...
			const after = readProfileState(profileDir);
			writeProfileState(profileDir, withBundleDisabled(after.manifest, packageName), withoutDisabled(after.plugins.disabledNames, packageName), withoutPackage(after.plugins.packages, entry.id));
